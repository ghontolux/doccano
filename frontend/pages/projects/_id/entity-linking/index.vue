...
import EntityItemBox from '~/components/tasks/entityLinking/EntityItemBox'
...
export default {
  ...
  methods: {
    async list(docId) {
      ...
      const annotations = await this.$services.entityLinking.list(this.projectId, docId);
    ...
    async remove(id) {
      await this.$services.entityLinking.delete(this.projectId, this.doc.id, id)
      await this.list(this.doc.id)
    },

    async add(startOffset, endOffset, entId) {
      await this.$services.entityLinking.create(this.projectId, this.doc.id, entId, startOffset, endOffset)
      await this.list(this.doc.id)
    },

    async update(labelId, annotationId) {
      await this.$services.entityLinking.changeLabel(this.projectId, this.doc.id, annotationId, labelId)
      await this.list(this.doc.id)
    },

    async clear() {
      await this.$services.entityLinking.clear(this.projectId, this.doc.id)
      await this.list(this.doc.id)
    },

    async autoLabel(docId) {
      try {
        await this.$services.entityLinking.autoLabel(this.projectId, docId)
      } catch (e) {
